...
const int TICKS_PER_REVOLUTION = 1024;  // 1024 імпульсів на оберт колеса
...
int main(int argc, char** argv) {
  ...
  std::ofstream outputFile(OUTPUT_FILE_PATH, ios::out);
  ...
  int timestamp_ms, fl_ticks, fr_ticks, bl_ticks, br_ticks;
  int prev_fl_ticks, prev_fr_ticks, prev_bl_ticks, prev_br_ticks;
  int index = 0;
  string line;
  ...
  while (getline(inputFile, line)) {
    ...
    if (index == 0) {
      ...
      continue;  // Пропускаємо перший елемент оскільки нам потрібен попередній зразок для обчислення різниць
    }

    // Крок 1. Delta iмпульсiв по кожному колесу:
    int d_fl = fl_ticks - prev_fl_ticks;
    int d_fr = fr_ticks - prev_fr_ticks;
    int d_bl = bl_ticks - prev_bl_ticks;
    int d_br = br_ticks - prev_br_ticks;
    ...
    int d_left = (d_fl + d_bl) / 2;
    int d_right = (d_fr + d_br) / 2;
